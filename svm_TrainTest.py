...
#
# ocsvm_classifier = svm.OneClassSVM(kernel='rbf', gamma='scale' , nu=0.05) # nu: describes classification line. Value is: b/w 0 and 1
# ocsvm_classifier.fit(features)
# print("OCSVM classifier is trained")
# filename = 'OCSVM_model12.sav'
# pickle.dump(ocsvm_classifier, open(filename, 'wb'))
# print("trained classifier is saved")

#Prediction phase
classifier = pickle.load(open("OCSVM_model12.sav", 'rb'))
test_score = []
# id = []
...
# res.to_csv("OCSVM_scoresNfc123.csv")
# print(res)
with open("out_0.json", 'r') as f:
    ...
    for i in range(loop):
        ...
        # print("// Image Type ///: ", image.size, type(image ))
        frame = cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB)
        # print("// Frame Type ///: ", frame.shape, type(frame))
        frame = Image.fromarray(frame)
        # print("// frame Type ///: ", frame.size, type(frame))
...
for i in range(len(test_score)):
    if test_score[i]<0:
        ...
        continue
acc = ((len(test_score)-count)/len(test_score))*100
print(acc)
print(count)